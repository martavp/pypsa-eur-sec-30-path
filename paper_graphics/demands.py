...
path = '../../postnetworks/'  
network_name= path+'postnetwork-go_opt_2025.nc'  
...
ax1.set_ylabel('GWh', fontsize=20)

ax1.fill_between(np.arange(0,8760), dem_heat_rural+dem_heat_urban, dem_heat_rural, color='orange',  label='urban heating', linewidth=0.5)
ax1.fill_between(np.arange(0,8760), dem_heat_rural, 0, color='red',  label='rural heating', linewidth=0.5)

ax1.fill_between(np.arange(0,8760), 0, dem_heat_cooling, color='blue',  label='cooling', linewidth=0.5, alpha=1)
ax1.plot(np.arange(0,8760), dem_electricity, color='black', label='electricity', linewidth=2, alpha=0.5)
...
ax1.set_xticklabels(['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'], fontsize=18)
ax1.legend(loc=(1.01,0.1), shadow=True, fancybox=True, ncol=1, prop={'size':18}) 
ax1.set_ylim(0,1000)
