...
#        raise RuntimeError
    ...
    if options['cooling_coupling']:
        ...
        def HP_nom(model,node):
            return model.link_p_nom[node+' heat pump'] == model.link_p_nom[node+' cooling pump']

        network.model.HP_nom = Constraint(nodes,rule=HP_nom)

        def HP_sum(model,node,snapshot):
            return model.link_p[node+' heat pump',snapshot]+model.link_p[node+' cooling pump',snapshot] <= model.link_p_nom[node+' heat pump']
