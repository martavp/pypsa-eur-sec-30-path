...
gs1.update(wspace=0.05, hspace=0.07)
...
for i,tech in enumerate(['solar', 'onwind', 'elec_demand', 'heat_demand']):    
    ...
    ax2.set_ylim(0,1.00)
    ax2.set_yticks([])
    ax2.legend(loc=(0.55, -0.05), shadow=True, fancybox=True, prop={'size':12}) #'center left'
    #ax2.set_yticks([0, 0.5, 1])
    #ax2.set_yticklabels(['0','0.5', '1'], fontsize=14)
    #ax2.set_xlabel('April,16 - May,3 2015', fontsize=14)
...
for i,tech in enumerate(['battery', 'PHS', 'H2 underground',  'LTES']):    
    ...
    ax2.legend(loc=(0.0, 0.0), shadow=True, fancybox=True, prop={'size':12}) #'center left'
    # ax2 = plt.subplot(gs1[1,1])
    # #ax2.text(0.9, 0.9, 'b)', transform=ax2.transAxes, fontsize=16)
    # ax2.set_xlim(1,10000)
    # ax2.set_ylim(0,1.00)
