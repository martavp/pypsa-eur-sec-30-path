...
\begin{figure*}[!h]
\centering
\includegraphics[width=0.8\textwidth]{figures/age_distribution_wo_DH_exp.png}
...
\begin{figure*}[!h]
\centering
\includegraphics[width=0.8\textwidth]{figures/heating_expansion_wo_DH_exp.png}
...
District heating has proven to be extremely useful to decarbonise the heating sector. It allows cheaper centralised heating technologies such as heat pumps, heat resistors, gas boilers and CHP units, and makes possible a fast conversion because it is easier to substitute one central heating unit that a myriad of individual domestic systems. On the top of that, district heating enables long-term thermal energy storage, via cheap large water pits, Fig. \ref{fog_Fourier} that help balancing the large seasonal variation of heating demand, Supplementary Note X. In the initial paths, district heating penetrations were kept fixed at their values in 2015 \cite{DH_penetration} throughout the entire path. \textcolor[rgb]{1,0,0}{When they are assumed to expand linearly so that all the urban heat demand in every country is supplied via district heating, system cost reduces by X B\EUR. Individual heat pump capacities are substituted by ... (estimar el coste del district heating).}
