...
def die_out(year):
    df_agg = pd.read_csv('data/existing_2020/existing_pp_2020.csv',
                         index_col=0, dtype={'Fueltype':str,'Country':str})
    countries = list(set(df_agg.Country))
    df_agg = df_agg.rename(columns={'Capacity': 'p_nom'})
    
    # capacities in pypsa are thermal (before conversion to electricity)
...
expand.drop(expand.filter(regex='gas|coal|lignite|nuclear|oil|biomass').index,
            inplace=True)
